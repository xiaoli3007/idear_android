...
public class ListTempleAdapter extends BaseAdapter implements OnClickListener {
	...
	public static class ListItem {
		...
		public Button createOrderBackground;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (null == convertView) {
			...
			listItem.createOrderBackground = (Button) convertView.findViewById(R.id.list_temple_listitem_create_order_background_button);
			convertView.setTag(listItem);
		} else {
			listItem = (ListItem) convertView.getTag();
		}
		JSONObject item = this.data.optJSONObject(position);
		ShangXiang.imageLoader.displayImage(item.optString("pic_tmb_path", ""), listItem.thumbHall, ShangXiang.imageLoaderOptions, new ImageLoadingListener() {
			@Override
		...
		ShangXiang.imageLoader.displayImage(item.optString("tmb_headface", ""), listItem.thumbBuddhist, ShangXiang.imageLoaderOptions, new ImageLoadingListener() {
			@Override
			public void onLoadingStarted(String imageUri, View view) {
		...
		listItem.createOrderBackground.setTag(item.toString());
		listItem.createOrderBackground.setOnClickListener(this);
		return convertView;
	}

	@Override
	public void onClick(View v) {
		Message msg = ShangXiang.listTempleHandler.obtainMessage(0, v.getTag());
		ShangXiang.listTempleHandler.sendMessage(msg);
	}
}